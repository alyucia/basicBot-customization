	...
	var basicBot = {
		...
		eventDjadvance: function(obj) {
			...
			if (basicBot.settings.songstats) {
				if (typeof basicBot.chat.songstatistics === "undefined") {
					API.sendChat("/me " + lastplay.media.author + " - " + lastplay.media.title + ": " + lastplay.score.positive + "W/" + lastplay.score.grabs + "G/" +
		...
		chatUtilities: {
			chatFilter: function(chat) {
				...
				if (msg.indexOf('autojoin was not enabled') > 0 || msg.indexOf('AFK message was not enabled') > 0 || msg.indexOf('!afkdisable') > 0 || msg.indexOf(
					'!joindisable') > 0 || msg.indexOf('autojoin disabled') > 0 || msg.indexOf('AFK message disabled') > 0) {
					API.moderateDeleteChat(chat.cid);
					return true;
				}
			...
			spam: ['hueh', 'hu3', 'brbr', 'heu', 'brbr', 'kkkk', 'spoder', 'mafia', 'zuera', 'zueira', 'zueria', 'aehoo', 'aheu', 'alguem', 'algum', 'brazil',
				'zoeira', 'fuckadmins', 'affff', 'vaisefoder', 'huenaarea', 'hitler', 'ashua', 'ahsu', 'ashau', 'lulz', 'huehue', 'hue', 'huehuehue', 'merda', 'pqp',
				'puta', 'mulher', 'pula', 'retarda', 'caralho', 'filha', 'ppk', 'gringo', 'fuder', 'foder', 'hua', 'ahue', 'modafuka', 'modafoka', 'mudafuka',
				'mudafoka', 'ooooooooooooooo', 'foda'
			],
			curses: ['nigger', 'fag', 'f@g', 'cunt', 'bitch', 'b1tch', 'bltch', 'nigga', 'n1gga', 'n1gg@', 'niqqa', 'n1qqa', 'nigg@', 'fuck3r', 'f@gs', 'motherfucker',
				'm0therfucker', 'm0therfuck3r', 'bastard', 'b@st@rd', 'ba5tard', 'b@stard', 'basterd', 'fuck', 'fucker', 'asshole', '@asshole', 'shit', 'sh1t', 'pussy',
				'pu55y', 'pu5sy', 'pus5y', 'pussi'
			]
		},
		connectAPI: function() {
			this.proxy = {
			...
			msgCommand: {
				...
				functionality: function(chat, cmd) {
					...
					if (!basicBot.commands.executable(this.rank, chat)) return void(0);
						...
						var msg = chat.message;
						if (msg.length <= cmd.length + 1) return API.sendChat('/me msg: ' + basicBot.settings.msg);
						var argument = msg.substring(cmd.length + 1);
						if (!basicBot.settings.msgEnabled) basicBot.settings.msgEnabled = !basicBot.settings.msgEnabled;
						if (isNaN(argument)) {
							basicBot.settings.msg = argument;
							API.sendChat(subChat(basicBot.chat.msgset, {
								msg: basicBot.settings.msg
							}));
						} else {
							basicBot.settings.msgInterval = argument;
							API.sendChat(subChat(basicBot.chat.msgintervalset, {
								interval: basicBot.settings.msgInterval
							}));
						}
					}
				}
			...
			togglemsgCommand: {
				...
				functionality: function(chat, cmd) {
					...
					if (!basicBot.commands.executable(this.rank, chat)) return void(0);
						...
						if (basicBot.settings.msgEnabled) {
							basicBot.settings.msgEnabled = !basicBot.settings.msgEnabled;
							API.sendChat(subChat(basicBot.chat.toggleoff, {
								name: chat.un,
								'function': basicBot.chat.msg
							}));
						} else {
							basicBot.settings.msgEnabled = !basicBot.settings.msgEnabled;
							API.sendChat(subChat(basicBot.chat.toggleon, {
							...
							if (random > 1) {
								...
								setTimeout(function() {
									...
									if (id1 === -1 || id2 === -1) API.sendChat(basicBot.chat.notonwaitlist);
										...
										API.moderateMoveDJ(basicBot.userUtilities.lookupUserName(basicBot.settings.fighter1).id, id2+1, false);
										API.moderateMoveDJ(basicBot.userUtilities.lookupUserName(basicBot.settings.fighter2).id, id1+1, false);
